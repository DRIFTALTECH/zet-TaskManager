...
def admin_login(body: AdminLoginBody, db: Db = Depends(get_db)):
...
def admin_login_microsoft(body: AdminMicrosoftLoginBody, db: Db = Depends(get_db)):
...
def change_role(user_id: str, body: AdminRoleUpdate, db: Db = Depends(get_db)):
    return admin_logic.change_role(db, user_id, body)


@router.post("/users/{user_id}/password", dependencies=[Depends(require_admin)])
def reset_password(user_id: str, body: AdminPasswordReset, db: Db = Depends(get_db)):
...
def set_projects(user_id: str, body: AdminProjectsUpdate, db: Db = Depends(get_db)):
    return admin_logic.set_projects(db, user_id, body)


@router.patch("/users/{user_id}/manager", response_model=UserOut, dependencies=[Depends(require_admin)])
def set_manager(user_id: str, body: AdminManagerUpdate, db: Db = Depends(get_db)):
    return admin_logic.set_manager(db, user_id, body)


@router.post("/users/{user_id}/deactivate", response_model=UserOut, dependencies=[Depends(require_admin)])
def deactivate(user_id: str, db: Db = Depends(get_db)):
...
def activate(user_id: str, db: Db = Depends(get_db)):
    return admin_logic.set_active(db, user_id, True)


@router.post("/users/{user_id}/delete", dependencies=[Depends(require_admin)])
def delete_user(user_id: str, body: AdminUserDelete, db: Db = Depends(get_db)):
...
def list_audit(limit: int = 200, db: Db = Depends(get_db)):
...
def change_admin_password(body: AdminChangePassword, db: Db = Depends(get_db)):
