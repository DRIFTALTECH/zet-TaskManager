...
def active_timers(user_id: str = Depends(get_current_user_id), db: Db = Depends(get_db)):
    """Running timers for the current user (so the UI shows live state after a reload)."""
    return timer_logic.list_active(db, user_id)


@router.get("/timers/min-persist", response_model=MinTimerPersistOut)
def get_min_persist(user_id: str = Depends(get_current_user_id), db: Db = Depends(get_db)):
    return MinTimerPersistOut(minutes=timer_logic.get_min_persist_minutes(db))


@router.put("/timers/min-persist", response_model=MinTimerPersistOut)
def put_min_persist(
    body: MinTimerPersistBody,
    user_id: str = Depends(get_current_user_id),
    db: Db = Depends(get_db),
):
    return timer_logic.set_min_persist_minutes(db, user_id, body.minutes)


@router.post("/{task_id}/timer/start", response_model=TimerRunOut)
def start_timer(task_id: str, user_id: str = Depends(get_current_user_id), db: Db = Depends(get_db)):
    return timer_logic.start(db, user_id, task_id)


@router.post("/{task_id}/timer/stop", response_model=TaskOut)
def stop_timer(task_id: str, body: TimerStopBody, user_id: str = Depends(get_current_user_id), db: Db = Depends(get_db)):
...
def create_task(body: TaskCreate, user_id: str = Depends(get_current_user_id), db: Db = Depends(get_db)):
    return task_logic.create_task_action(db, user_id, body)


@router.patch("/{task_id}", response_model=TaskOut)
def patch_task(task_id: str, body: TaskPatch, user_id: str = Depends(get_current_user_id), db: Db = Depends(get_db)):
    return task_logic.patch_task_action(db, user_id, task_id, body)


@router.delete("/{task_id}", status_code=204)
def delete_task(task_id: str, user_id: str = Depends(get_current_user_id), db: Db = Depends(get_db)):
    task_logic.delete_task_action(db, user_id, task_id)
    return Response(status_code=204)


@router.post("/{task_id}/start", response_model=TaskOut)
def start_task(task_id: str, user_id: str = Depends(get_current_user_id), db: Db = Depends(get_db)):
    return task_logic.start_task_action(db, user_id, task_id)


@router.post("/{task_id}/move", response_model=TaskOut)
def move_task(task_id: str, body: TaskMoveBody, user_id: str = Depends(get_current_user_id), db: Db = Depends(get_db)):
    return task_logic.move_task_action(db, user_id, task_id, body)


@router.post("/{task_id}/reopen-to-backlog", response_model=TaskOut)
def reopen_task_to_backlog(task_id: str, user_id: str = Depends(get_current_user_id), db: Db = Depends(get_db)):
...
def log_time(task_id: str, body: LogTimeBody, user_id: str = Depends(get_current_user_id), db: Db = Depends(get_db)):
    return task_logic.log_time(db, user_id, task_id, body)


@router.get("/{task_id}/feedback", response_model=list[TaskFeedbackOut])
def list_task_feedback(task_id: str, user_id: str = Depends(get_current_user_id), db: Db = Depends(get_db)):
    return task_feedback_logic.list_feedback(db, user_id, task_id)


@router.post("/{task_id}/feedback", response_model=TaskFeedbackOut)
def create_task_feedback(task_id: str, body: TaskFeedbackCreate, user_id: str = Depends(get_current_user_id), db: Db = Depends(get_db)):
...
def patch_task_feedback(task_id: str, feedback_id: str, body: TaskFeedbackPatch, user_id: str = Depends(get_current_user_id), db: Db = Depends(get_db)):
