/**
 * User Overview body — filters live in the shared Overview header.
 */
...
export default function UserOverviewPanel({
  ...
  const { data, isLoading, error } = useQuery({
    queryKey: ['user-overview', userId, status, projectId],
    queryFn: () => analyticsExtApi.getUserOverview(
      userId,
      status,
  ...
  if (!userId) {
    return (
      <p className="text-sm text-muted-foreground py-16 text-center">
        No people available in your projects.
      </p>
    );
  }
      ...
      {data && (
        <>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            <AnalyticsKpiCard icon={ListTodo} label="Tasks" value={data.summary.total} sub={data.userName} />
